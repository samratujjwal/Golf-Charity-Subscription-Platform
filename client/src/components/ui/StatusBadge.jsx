export default function StatusBadge({ status, children }) {
  ...
  const styleClass = styles[normalizedStatus] || 'border-indigo-400/30 bg-indigo-500/12 text-indigo-300';

  return (
    <span className={`inline-flex items-center rounded-full border px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.22em] ${styleClass}`}>
      {label}
    </span>
  );
}
