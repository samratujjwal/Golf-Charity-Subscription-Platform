...
export default function DashboardWinnings() {
  ...
  const totalPrizeAmount = winnings.reduce((sum, winning) => sum + Number(winning.prizeAmount || 0), 0);
  ...
  const handleFileChange = async (winningId, event) => {
    ...
    try {
      ...
      setError(uploadError.response?.data?.error || uploadError.message || 'Unable to upload proof');
    } finally {
      event.target.value = '';
    }
  };
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-white/50 bg-[radial-gradient(circle_at_top_right,rgba(250,204,21,0.18),transparent_26%),linear-gradient(135deg,rgba(255,255,255,0.96),rgba(248,250,252,0.92))] p-8 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur">
        <p className="text-xs font-semibold uppercase tracking-[0.28em] text-slate-500">Winnings</p>
        <h2 className="mt-3 text-4xl font-semibold tracking-tight text-slate-950">Track every reward, proof review, and payout milestone.</h2>
        <p className="mt-4 max-w-2xl text-base leading-8 text-slate-600">
          Once a draw completes and prize distribution runs, your winnings appear here with proof upload and payout status updates.
        </p>
      </section>

      <div className="grid gap-4 md:grid-cols-3">
        <DashboardCard title="Total Winnings" value={`$${totalPrizeAmount.toFixed(2)}`} description="Combined prize value across all current winnings." />
        <DashboardCard title="Open Reviews" value={String(winnings.filter((winning) => winning.status === 'pending').length)} description="Pending winnings waiting for your proof or admin verification." />
        <DashboardCard title="Paid Out" value={String(winnings.filter((winning) => winning.status === 'paid').length)} description="Winnings that have completed the payout workflow." />
      </div>
      ...
      {winningsQuery.isError && (
        <div className="rounded-[2rem] border border-rose-200 bg-rose-50 p-6 text-rose-700 shadow-[0_20px_70px_rgba(244,63,94,0.12)]">
          {winningsQuery.error?.response?.data?.error || 'Unable to load winnings.'}
        </div>
      )}

      {!winningsQuery.isLoading && !winningsQuery.isError && winnings.length === 0 && (
        <DashboardCard title="No winnings yet" description="When you match three or more draw numbers, your winning records and proof workflow will appear here." />
      )}
              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-3">
                    <h3 className="text-2xl font-semibold tracking-tight text-slate-950">
                      {winning.matchCount} matches for {winning.draw ? `${winning.draw.month}/${winning.draw.year}` : 'recent draw'}
                    </h3>
                    <StatusBadge status={winning.status} />
                  </div>
                  <p className="text-sm text-slate-600">
                    Prize amount: <span className="font-semibold text-slate-950">${Number(winning.prizeAmount).toFixed(2)}</span>
                  </p>
                ...
                <div className="min-w-[240px] space-y-3">
                  {winning.proofImage ? (
                    ...
                    >
                      <img src={winning.proofImage} alt="Proof" className="h-40 w-full object-cover" />
                    </a>
                  ) : (
                  ...
                  <button
                    ...
                    disabled={uploadProofMutation.isPending || winning.status !== 'pending'}
                  ...
                  >
                    {winning.proofImage ? 'Replace proof image' : 'Upload proof image'}
                  </button>
                </div>
              </div>
            </article>
