...
export default function DashboardScores() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError(getErrorMessage(submissionError, "Unable to save score"));
    }
  };

  const handleEdit = (score, index) => {
    setEditingIndex(index);
    setForm({ value: String(score.value), date: score.date.slice(0, 10) });
    setError("");
  };
    ...
    <div className="space-y-6">
      <section className="rounded-[2rem] border border-white/50 bg-white/85 p-8 shadow-[0_24px_80px_rgba(15,23,42,0.08)] backdrop-blur">
        ...
        <p className="mt-4 max-w-2xl text-base leading-8 text-slate-600">
          Store your latest five Stableford rounds. Values must be 1–45. The
          oldest is replaced automatically when you exceed five.
        </p>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-wrap gap-3">
              <button
                type="submit"
                disabled={
                  addScoreMutation.isPending || editScoreMutation.isPending
                }
        ...
        >
          ...
          {scoresQuery.isError && (
            <div className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">
              {getErrorMessage(scoresQuery.error, "Unable to load scores")}
            </div>
          )}
          {!scoresQuery.isLoading &&
            !scoresQuery.isError &&
            scores.length === 0 && (
              <div className="rounded-[1.5rem] border border-dashed border-slate-300 bg-slate-50 px-5 py-10 text-center text-slate-600">
                No scores yet. Add your first Stableford round to start the
                history.
              </div>
            )}
          {!scoresQuery.isLoading &&
                    ...
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                      <div>
                        <p className="text-xs font-semibold uppercase tracking-[0.22em] text-slate-500">
                          {index === 0
                            ? "Latest score"
                            : `Stored score ${scores.length - index}`}
                        </p>
