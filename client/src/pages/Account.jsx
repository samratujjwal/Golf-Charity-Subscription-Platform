...
export default function Account() {
  ...
  const handleRefreshProfile = async () => {
    ...
    try {
      ...
      setError(
        refreshError.response?.data?.error || "Unable to refresh profile",
      );
    } finally {
  ...
  return (
    <section className="mx-auto max-w-4xl rounded-[2rem] border border-white/40 bg-white/85 p-8 shadow-[0_24px_80px_rgba(20,57,44,0.16)] backdrop-blur">
      ...
      <h1 className="mt-3 font-serif text-4xl text-stone-900">
        Membership and billing overview
      </h1>
      <p className="mt-3 text-stone-600">
        Authentication is handled with short-lived access tokens, and premium
        access depends on the verified subscription record in the database.
      </p>
        ...
        {subscription ? (
          ...
          <div className="mt-3 flex flex-wrap items-center gap-3">
            <p className="text-stone-600">
              No active subscription is attached to this account yet.
            </p>
            <Link
              to="/pricing"
