...
export class WinningService {
  ...
  async listAdminWinnings(filters = {}) {
    ...
    return winnings.map((winning) => this.serializeWinning(winning, { includeUser: true, includeDraw: true }));
  }
  ...
  async calculatePrizePool(drawId) {
    ...
    const subscriptions = await this.winningRepository.getEligibleSubscriptionsForMonth({ monthStart, monthEnd });
    const previousDistribution = await this.winningRepository.getLatestDistributionBeforeCycle({
      year: draw.year,
      month: draw.month,
    });

    const basePoolCents = subscriptions.reduce((sum, subscription) => {
      const amountCents = this.toCents(subscription.amount);
      return sum + Math.round((amountCents * DEFAULT_POOL_PERCENTAGE) / 100);
    }, 0);
    const carryForwardCents = this.toCents(previousDistribution?.jackpotCarryForwardAmount || 0);
  ...
  async distributePrizes(drawId) {
    ...
    if (draw.status !== 'completed') {
      throw new ApiError(409, 'Prize distribution can only run after the draw is completed');
    }
    ...
    const aggregation = await this.winningRepository.aggregateWinningsByDraw(drawId);
    ...
    MATCH_TIERS.forEach((tier) => {
      ...
      const tierAllocationCents = Math.round((totalPoolCents * rule.percentage) / 100);
      const amountPerWinnerCents = winnersInTier.length > 0 ? Math.floor(tierAllocationCents / winnersInTier.length) : 0;
    ...
    const distributionRecord = await this.winningRepository.upsertDistributionByDrawId(drawId, {
      month: draw.month,
      year: draw.year,
      poolPercentage: DEFAULT_POOL_PERCENTAGE,
    ...
    const refreshedWinners = await this.winningRepository.getWinningsByDraw(drawId);
    ...
    return {
      ...
      tierBreakdown,
      winnings: refreshedWinners.map((winning) => this.serializeWinning(winning, { includeUser: true, includeDraw: true })),
  ...
  async uploadProof({ userId, winningId, imageData }) {
    ...
    if (!winning) {
      throw new ApiError(404, 'Winning entry not found');
    }

    if (winning.userId._id.toString() !== userId) {
      throw new ApiError(403, 'You can only upload proof for your own winnings');
    }

    if (winning.status !== 'pending') {
      throw new ApiError(409, 'Proof can only be uploaded while the winning is pending review');
    }
    ...
    const updatedWinning = await this.winningRepository.updateWinningProof(winningId, uploadResponse.secure_url);
    return this.serializeWinning(updatedWinning, { includeUser: true, includeDraw: true });
  }
  ...
  async verifyWinning(winningId) {
    ...
    if (winning.status === 'paid') {
      throw new ApiError(409, 'Paid winnings cannot be verified again');
    }

    const updatedWinning = await this.winningRepository.updateWinningStatus(winningId, 'verified');
    return this.serializeWinning(updatedWinning, { includeUser: true, includeDraw: true });
  }
  ...
  async markWinningPaid(winningId) {
    ...
    if (this.toCents(winning.prizeAmount) <= 0) {
      throw new ApiError(409, 'Prize distribution must run before marking a winning as paid');
    }

    const updatedWinning = await this.winningRepository.updateWinningStatus(winningId, 'paid');
    return this.serializeWinning(updatedWinning, { includeUser: true, includeDraw: true });
  }
  ...
  buildTierShares(totalPoolCents) {
    return MATCH_TIERS.map((tier) => {
      ...
      return {
        ...
        percentage: rule.percentage,
        rollover: rule.rollover,
        amount: this.fromCents(Math.round((totalPoolCents * rule.percentage) / 100)),
      };
    });
  }
  ...
  serializeWinning(winning, options = {}) {
    ...
    return {
      ...
      userId: winning.userId?._id ? winning.userId._id.toString() : winning.userId.toString(),
      drawId: winning.drawId?._id ? winning.drawId._id.toString() : winning.drawId.toString(),
      matchCount: winning.matchCount,
      prizeAmount: this.fromCents(this.toCents(winning.prizeAmount || 0)),
      status: winning.status,
      proofImage: winning.proofImage,
      createdAt: winning.createdAt,
      updatedAt: winning.updatedAt,
      user: includeUser && winning.userId?._id
        ? {
        ...
        : null,
      draw: includeDraw && winning.drawId?._id
