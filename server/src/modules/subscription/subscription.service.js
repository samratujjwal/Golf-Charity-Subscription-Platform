...
export class StripeSubscriptionService {
  ...
  async createCheckoutSession({ userId, plan }) {
    ...
    if (!user.charityId) {
      throw new ApiError(
        400,
        "Please select a charity before starting a subscription",
      );
    }
  ...
  async syncSubscriptionFromCheckoutSession({ userId, sessionId }) {
    ...
    const stripeSubscription = await this.fetchStripeSubscription(
  ...
  async handleInvoicePaymentSucceeded(invoice) {
    ...
    const existingPayment =
      await this.subscriptionRepository.findPaymentByTransactionId(
    ...
    const user =
      await this.resolveUserFromStripeSubscription(stripeSubscription);
    const subscriptionRecord = await this.syncSubscriptionRecord({
      user,
      stripeSubscription,
    ...
    if (user.charityId && existingPayment?.status !== "success") {
      await this.subscriptionRepository.incrementCharityDonations(
        ...
        this.calculateContribution(
          ...
          subscriptionRecord.charityPercentage,
        ),
      );
    }
  }
  ...
  async handleInvoicePaymentFailed(invoice) {
    ...
    const subscriptionRecord =
      await this.subscriptionRepository.findSubscriptionByStripeSubscriptionId(
        invoice.subscription,
      );
    if (!subscriptionRecord) return;

    await this.subscriptionRepository.updateSubscriptionById(
    ...
    // Fetch user to send expiry email
    try {
      const user = await this.subscriptionRepository.findUserById(
        subscriptionRecord.userId.toString(),
      );
      if (user) {
        await emailService.sendSubscriptionEnded({
          to: user.email,
          userName: user.name,
          plan: subscriptionRecord.plan,
          reason: "expired due to failed payment",
        });
      }
    } catch (_) {
      /* non-fatal */
  ...
  async syncSubscriptionRecord({
    ...
    if (!plan)
      throw new ApiError(
        500,
        "Unable to determine subscription plan from Stripe data",
      );
    ...
    const subscriptionRecord =
      await this.subscriptionRepository.upsertSubscriptionByStripeSubscriptionId(
        stripeSubscription.id,
        subscriptionPayload,
      );
  ...
  async resolveUser({ userId, email }) {
    ...
    if (!user && email) {
      user = await this.subscriptionRepository.findUserByEmail(
        email.toLowerCase(),
      );
    }
    if (!user)
  ...
  async resolveUserFromStripeSubscription(stripeSubscription) {
    ...
    return this.resolveUser({ userId: null, email });
  }
